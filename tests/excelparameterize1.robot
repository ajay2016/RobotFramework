...
Library             DataDriver       file=resources/data.xlsx     sheet_name=data
...
verify the error message is correct
    ${result_text}=  Get Text    ${LOGIN_ERROR_MSG}
    # from builtin library
    Should Be Equal As Strings      ${result_text}          Incorrect username/password.
    Element Text Should Be          ${LOGIN_ERROR_MSG}      Incorrect username/password.
